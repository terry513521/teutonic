...
def fetch_king() -> dict:
    ...
    with urllib.request.urlopen(DASHBOARD_URL, timeout=30) as r:
        d = json.loads(r.read())
    k = d["king"]
    repo = k.get("hf_repo") or k.get("model_repo")
    if not repo:
        raise KeyError(f"dashboard king missing repo field; keys={sorted(k.keys())}")
    k["hf_repo"] = repo
    revision = k.get("king_revision") or k.get("revision")
